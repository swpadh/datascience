...
object BLAS extends Serializable{
  ...
  @transient private lazy val log = LogManager.getLogger("BLAS")
...
*/
  ...
  def gemm(
    ...
    if (alpha == 0.0 && beta == 1.0) {
      log.debug("gemm: alpha is equal to 0 and beta is equal to 1. Returning C.")
    } else if (alpha == 0.0) {
 ...
 def gemm(
  ...
  def gemv(
    ...
    if (alpha == 0.0 && beta == 1.0) {
      log.debug("gemv: alpha is equal to 0 and beta is equal to 1. Returning y.")
    } else if (alpha == 0.0) {
      scal(beta, y)
    } else {
      (A, x) match {
